...
export default class Gantt {
    ...
    change_view_mode(mode = this.options.view_mode) {
        ...
        this.trigger_event('view_change', [mode]);
            // 动态调整 SVG 高度
        //const requiredHeight = this.tasks.length * (this.options.bar_height + this.options.padding) + this.options.header_height;
        //$.attr(this.$svg, { height: requiredHeight });
    }
    ...
    render() {
        ...
        */
        ...
        // 功能: 获取Bar类中的最大y值
        const maxY = Bar.max_y;

        // 功能: 定义额外的空白高度
        const extraPadding = 50; // 例如，增加50像素的空白

        // 功能: 根据最大y值和额外空白设置甘特图的高度
        const totalHeight = maxY + this.options.padding + extraPadding; 
        this.$svg.setAttribute('height', totalHeight);

        console.log("Rendering Gantt chart0ok");
    }
    ...
    */
    ...
    // Gantt 类中调整 show_simple_popup 方法
    // Gantt 类中调整 show_simple_popup 方法
    show_simple_popup(options) {
        if (!this.popup) {
            this.popup = new Popup(this.popup_wrapper);
        }

        // 重置弹出框的内容为简化版本
        this.popup.parent.innerHTML = `
            <div class="title" id="simple-popup-title">${options.title}</div>
        `;

        // 设置深灰色背景、白色文字、宽度样式和黄色边框
        this.popup.parent.style.background = '#333';  // 深灰色背景
        this.popup.parent.style.color = '#fff';       // 白色字体
        this.popup.parent.style.whiteSpace = 'nowrap'; // 防止无换行符时自动换行
        this.popup.parent.style.border = '2px solid #FFD700'; // 黄色边框

        // 使用临时元素测量文本宽度
        const tempElement = document.createElement('div');
        tempElement.style.fontSize = '12px'; // 根据实际情况调整字体大小
        tempElement.style.position = 'absolute';
        tempElement.style.whiteSpace = 'nowrap';
        tempElement.style.visibility = 'hidden';
        tempElement.textContent = options.title;
        ...
        // 动态设置弹出框宽度
        this.popup.parent.style.width = `${textWidth + 20}px`; // 添加一些额外宽度以留出边距

        // 调整位置
        const target_element = options.target_element;
        if (target_element instanceof SVGElement) {
            const position_meta = target_element.getBBox();
            this.popup.parent.style.left = (position_meta.x + position_meta.width + 10) + 'px'; // 在任务条右侧显示
            this.popup.parent.style.top = position_meta.y + 'px';
        }

        // 显示弹出框
        this.popup.parent.style.opacity = 1;
    }
        // Gantt 类中添加 hide_simple_popup 方法
    hide_simple_popup() {
        if (this.popup) {
            this.popup.hide(); // 隐藏弹出框
        }
    }
